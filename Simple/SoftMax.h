...
static double prob(const int t, const float* &x, const float** &weights) {
...
static double nk_error(int n, int k, const int* &ts, const float** &xs, const float** &weights) {
...
static int classify(const float* &x, const float** &weights) { // t of maximum probability P(t|x,w's) given predetermined w's.
...
static int error_of(const int correct_t, const float* &x, const float** &weights) {
